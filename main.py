...
def interpret_attachment(anxiety, avoidance):
    ...
    # 6 и 7. ТРЕВОЖНО-ИЗБЕГАЮЩИЙ (Смешанный тип)
    else:
        # Если оба параметра в среднем диапазоне
        if anxiety <= 83 and avoidance <= 83:
            ...
            # Если хотя бы один параметр высокий (>= 84), но при этом второй тоже > 41
            result_text = (
                "Ваш профиль соответствует тревожно-избегающему стилю "
...
async def start_handler(message: types.Message):
    ...
    welcome_text = (
        "📋 **Это опросник стилей привязанности в близких отношениях.**\n\n"
        ...
        "2. Интерпретацию вашего профиля\n"
        "3. Рекомендации по улучшению отношений исходя из ваших результатов\n"
        "4. Возможность отслеживать динамику. Для этого сделайте скриншот ваших результатов.\n\n"
        ...
        "🎯 **Как пройти тест:**\n\n"
        "Вам будет предложено по одному 36 утверждений.\n"
        "Оцените, насколько каждое утверждение описывает вас в отношениях.\n"
        "Здесь нет правильных и неправильных ответов.\n"
        "Ориентируйтесь на ваши чувства, а не на \"как должно быть\". Вспоминайте ваши последние значимые отношения.\n"
        "Выберете одну из семи оценок для каждого из утверждений.\n\n"
        "📝 **Шкала оценки:**\n\n"
...
async def back_handler(call: types.CallbackQuery):
    ...
    if uid in user_index and user_index[uid] > 0:
        user_index[uid] -= 1
        user_answers[uid].pop() # Удаляем последний ответ
...
async def answer_handler(call: types.CallbackQuery):
    ...
    if user_index[uid] < 36:
        ...
        await call.message.answer(result_message, parse_mode="Markdown", disable_web_page_preview=True)
